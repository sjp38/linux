...
``update_schemes_tried_bytes`` to ``state`` file updates the DAMON-based
operation scheme action tried regions directory for each DAMON-based operation
scheme of the kdamond, but only for ``total_bytes`` file.  Writing
...
Each filter directory contains three files, namely ``type``, ``matcing``,
``memcg_path``, ``addr_start``, and ``addr_end``.  You can write one of three
special keywords, ``anon`` for anonymous pages, ``memcg`` for specific memory
cgroup, or ``addr`` for specific address range (end-open interval) filtering.
...
specify the start and end address of the filter to ``addr_start`` and
``addr_end`` files, respectively.  You can write ``Y`` or ``N`` to ``matching``
file to filter out pages that does or does not match to the type, respectively.
Then, the scheme's action will not be applied to the pages that specified to be
filtered out.
...
Also, memory regions that filtered out by ``addr`` filters are not accounted as
the scheme has tried at all, while those that filtered out by other types are
accounted as the scheme has tried, to :ref:`stats <damos_stats>` and
...
Writing ``update_schemes_tried_bytes`` to the relevant ``state`` file doesn't
create the sub directories, but only update the ``total_bytes`` file.
