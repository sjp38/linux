...
user inputs on the sysfs files other than the ``state`` file again.  Writing
...
``avail_operations`` file.  You can set and get what type of monitoring
operations DAMON will use for the context by writing one of below keywords to,
and reading from the ``operations`` file.

 - vaddr: Monitor virtual address spaces of specific processes
 - fvaddr: Monitor fixed ranges of specific virtual address spaces
...
(``fvaddr`` or ``paddr`` is written to the ``contexts/<N>/operations``).
