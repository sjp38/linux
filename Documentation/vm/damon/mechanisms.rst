...
primitives that dependant on and optimized for the target address space.  On
...
bit(s) set again after one sampling period.  To avoid disturbing other Accessed
bit users such as the reclamation logic, the implementations adjust the
``PG_Idle`` and ``PG_Young`` appropriately, as same to the 'Idle Page
Tracking'.
