...
/*
 * Discardable memory(dmem) store and load easily discardable pages for gcma
 * second-class clients. It enhances system memory space efficiency by
 * utilizing gcma area and helps guaranteeing both success and fast latency for
 * first-class client(contig memory allocation, of course) by discarding pages
 * in it when first-class client needs them.
 */
