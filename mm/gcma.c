...
/*
 * frontswap_alloc_page - Allocates a page for frontswap backend
 *
 ...
 */
static unsigned long complete_isolation(struct gcma *gcma,
...
retry:
	...
	start_pfn = complete_isolation(gcma, orig_start, orig_start + size);
	if (start_pfn)
		goto retry;
