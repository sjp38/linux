...
struct swap_slot_entry {
	struct gcma *gcma;	/* gcma where @page belongs in */
 ...
 *
 * GF_SWAP_LRU
 * The page is being used for frontswap and hang on frontswap LRU list. It can
 * be drained for contiguous memory allocation anytime.
 * Protected by slru_lock.
 *
 * GF_RECLAIMING
 * The page is being draining for contiguous memory allocation. Frontswap
 * guests should not use it.
 * Protected by slru_lock.
 *
 * GF_ISOLATED
 * The page is isolated for contiguous memory allocation. Contiguous allocation
 * guests can use the page safely, while Frontswap guests should not use it.
...
/*
 * Return 0 if [start_pfn, end_pfn] is isolated. Otherwise, return first unisolated pfn
 */
