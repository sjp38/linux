...
/*
 * damon_adjust_region_ranges() - Adjust regions of a target for new monitoring
 *				  target ranges.
 * @t:		the given target.
 ...
 * ranges.
 */
static void damon_adjust_region_ranges(struct damon_target *t,
		struct damon_addr_range *ranges, unsigned int nr_ranges)
{
	...
	/* Adjust intersecting regions to fit with the new ranges */
	for (i = 0; i < nr_ranges; i++) {
		struct damon_region *first = NULL, *last, *newr;
		struct damon_addr_range *range;

		range = &ranges[i];
		/* Get the first and last regions which intersect with range */
		...
		if (!first) {
			/* no damon_region intersects with this range */
...
{
	damon_adjust_region_ranges(t, bregions, 3);
}
