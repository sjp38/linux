			...
			struct damos_watermarks *wmarks)
{
	...
	/* next_apply_sis will be set in damon_set_attrs() or kdamond_fn() */
		...
		struct damon_attrs *old_attrs, struct damon_attrs *new_attrs)
{
	r->nr_accesses = damon_nr_accesses_for_new_attrs(r->nr_accesses,
			old_attrs, new_attrs);
	r->moving_accesses_bp = r->nr_accesses * 10000;
...
int damon_set_attrs(struct damon_ctx *ctx, struct damon_attrs *attrs)
{
	...
	/* adjust scheme->apply_interval_us */
...
{
	unsigned long sz;
	unsigned int nr_accesses = r->moving_accesses_bp / 10000;
		...
		struct damon_region *l, struct damon_region *r)
{
	...
	l->nr_accesses = (l->nr_accesses * sz_l + r->nr_accesses * sz_r) /
			(sz_l + sz_r);
	l->moving_accesses_bp = l->nr_accesses * 10000;
				  ...
				  struct damon_region *r, unsigned long sz_r)
{
	...
	new->moving_accesses_bp = r->moving_accesses_bp;
...
static void kdamond_init_intervals_sis(struct damon_ctx *ctx)
{
	...
	damon_for_each_scheme(scheme, ctx) {
		scheme->next_apply_sis =
			(scheme->apply_interval_us ? scheme->apply_interval_us
 ...
 * @nomvsum:	Non-moving sum of the last @len_window aligned @len_window
 * 		values.
 ...
 * have 25 values, @nomvsum is the sum of the 11th to 20th values values of the
 * 25 values.  Hence, this function simply drops @nomvsum / @len_window from
		...
		struct damon_attrs *attrs)
{
	...
	r->moving_accesses_bp = damon_moving_sum(r->moving_accesses_bp,
