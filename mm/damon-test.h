...
static void damon_test_str_to_pids(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, (unsigned long)123, answers[0]);
	kfree(answers);

	question = "123abc";
	answers = str_to_pids(question, strnlen(question, 128), &nr_integers);
	KUNIT_EXPECT_EQ(test, (ssize_t)1, nr_integers);
	KUNIT_EXPECT_TRUE(test, 123 == answers[0]);
	kfree(answers);

	question = "a123";
	answers = str_to_pids(question, strnlen(question, 128), &nr_integers);
	KUNIT_EXPECT_TRUE(test, nr_integers == 0);
	KUNIT_EXPECT_TRUE(test, answers == NULL);
...
static void damon_test_regions(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, (unsigned long)1, r->vm_start);
	KUNIT_EXPECT_EQ(test, (unsigned long)2, r->vm_end);
	KUNIT_EXPECT_EQ(test, (unsigned int)0, r->nr_accesses);
	KUNIT_EXPECT_TRUE(test, r->sampling_addr >= r->vm_start);
	KUNIT_EXPECT_TRUE(test, r->sampling_addr < r->vm_end);

	t = damon_new_task(42);
	KUNIT_EXPECT_EQ(test, (unsigned int)0, nr_damon_regions(t));

	damon_add_region_tail(r, t);
	KUNIT_EXPECT_EQ(test, (unsigned int)1, nr_damon_regions(t));
...
static void damon_test_tasks(struct kunit *test)
{
	...
	t = damon_new_task(42);
	KUNIT_EXPECT_EQ(test, (unsigned long)42, t->pid);
	KUNIT_EXPECT_EQ(test, (unsigned int)0, nr_damon_tasks());

	damon_add_task_tail(t);
	KUNIT_EXPECT_EQ(test, (unsigned int)1, nr_damon_tasks());
...
static void damon_test_three_regions_in_vmas(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, (unsigned long)10, regions[0].start);
	KUNIT_EXPECT_EQ(test, (unsigned long)25, regions[0].end);
	KUNIT_EXPECT_EQ(test, (unsigned long)200, regions[1].start);
	KUNIT_EXPECT_EQ(test, (unsigned long)220, regions[1].end);
	KUNIT_EXPECT_EQ(test, (unsigned long)300, regions[2].start);
	KUNIT_EXPECT_EQ(test, (unsigned long)330, regions[2].end);
}
...
static void damon_test_write_rbuf(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, damon_rbuf_offset, (unsigned int)5);
...
static void damon_test_update_two_gaps(struct kunit *test)
{
	...
	damon_for_each_region(r, t) {
		if (prev == NULL) {
			KUNIT_EXPECT_EQ(test, r->vm_start, (unsigned long)5);
			goto next;
		}

		if (prev->vm_end != r->vm_start && first_gap) {
			KUNIT_EXPECT_EQ(test, prev->vm_end, (unsigned long)27);
			KUNIT_EXPECT_EQ(test, r->vm_start, (unsigned long)45);
			first_gap = false;
			goto next;
		}

		if (prev->vm_end != r->vm_start && !first_gap) {
			KUNIT_EXPECT_EQ(test, prev->vm_end, (unsigned long)55);
			KUNIT_EXPECT_EQ(test, r->vm_start, (unsigned long)73);
			goto next;
		}
...
static void damon_test_split(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, nr_damon_regions(t), (unsigned int)10);
	...
	KUNIT_EXPECT_EQ(test, nr_damon_regions(t), (unsigned int)5);
	...
	KUNIT_EXPECT_EQ(test, r->vm_start, 5 + 10 * i);
	KUNIT_EXPECT_EQ(test, r->vm_end, (unsigned long)59);
	damon_free_task(t);
	...
	KUNIT_EXPECT_EQ(test, nr_damon_regions(t), (unsigned int)1);

	damon_for_each_region(r, t) {
		KUNIT_EXPECT_EQ(test, r->vm_start, (unsigned long)5);
		KUNIT_EXPECT_EQ(test, r->vm_end, (unsigned long)6);
	}
	damon_free_task(t);
}
...
static void damon_test_split_at(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, r->vm_start, (unsigned long)0);
	KUNIT_EXPECT_EQ(test, r->vm_end, (unsigned long)25);

	r = damon_next_region(r);
	KUNIT_EXPECT_EQ(test, r->vm_start, (unsigned long)25);
	KUNIT_EXPECT_EQ(test, r->vm_end, (unsigned long)100);
...
static void damon_test_merge_two(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, r->vm_start, (unsigned long)0);
	KUNIT_EXPECT_EQ(test, r->vm_end, (unsigned long)300);
	KUNIT_EXPECT_EQ(test, r->nr_accesses, (unsigned int)16);
...
static void damon_test_split_regions_of(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, nr_damon_regions(t), (unsigned int)2);
	damon_free_task(t);
}
