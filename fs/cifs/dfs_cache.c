...
static void refresh_cache(struct cifs_ses **sessions)
{
	...
	for (i = 0; i < CACHE_HTABLE_SIZE; i++) {
		...
		hlist_for_each_entry(ce, l, hlist) {
			if (count == ARRAY_SIZE(referrals))
				goto out_unlock;
			if (hlist_unhashed(&ce->hlist) || !cache_entry_expired(ce))
				continue;
			referrals[count].path = kstrdup(ce->path, GFP_ATOMIC);
			referrals[count++].ses = find_ipc_from_server_path(sessions, ce->path);
		}
	}
